// src/pages/Reservation.jsx → FINAL 100% WORKING VERSION
import { useState } from "react";
import { useNavigate } from "react-router-dom"; // ← FIXED: only one import
...
// Check if two time slots overlap (90-minute rule)
const isOverlapping = (existingTime, newTime) => {
  const toMin = t => t.split(":").reduce((h, m) => h * 60 + +m, 0);
  const s1 = toMin(existingTime), e1 = s1 + DURATION;
  const s2 = toMin(newTime),     e2 = s2 + DURATION;
  return s1 < e2 && s2 < e1;
};
...
export default function Reservation() {
  ...
  const { allReservations = [], createReservation } = useReservation();
  const notify = useNotification();
  const navigate = useNavigate();

  const today = new Date().toISOString().split("T")[0];

  // Check if a SPECIFIC table is already booked for the selected date & time
  ...
  const handleProceed = (e) => {
    ...
    if (isTableBooked(selectedTable.number)) {
      notify("This table was just taken! Please choose another.", "error");
      return;
    }
  ...
  const confirmAndPay = async () => {
    ...
    try {
      ...
      notify("Table no longer available. Please try again.", "error");
    }
  };
        ...
        <div className="w-full max-w-xl lg:max-w-2xl lg:mr-10">
          <div className="bg-white/97 backdrop-blur-xl rounded-3xl shadow-2xl p-10 lg:p-12 border border-white/40">
            ...
            <form onSubmit={handleProceed} className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    min={today}
                    value={date}
                    onChange={e => { setDate(e.target.value); setTime(""); setSelectedTable(null); }}
                    className="w-full px-6 py-4 rounded-2xl bg-[#FFF8F0] text-[#5C3A2E] focus:outline-none focus:ring-4 focus:ring-[#5C3A2E]/20"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    max="20"
                    value={guests}
                    onChange={e => { setGuests(e.target.value); setSelectedTable(null); }}
                    className="w-full px-6 py-4 rounded-2xl bg-[#FFF8F0] text-[#5C3A2E] focus:outline-none focus:ring-4 focus:ring-[#5C3A2E]/20"
                    required
                  />
                </div>
              ...
              {/* TABLE GRID — ONLY BOOKED TABLES ARE MARKED */}
              <div className="mt-10">
                ...
                {date && time ? (
                  <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-5">
                    {tables.map(table => {
                      const isBooked = isTableBooked(table.number);
                      const isTooSmall = table.capacity < Number(guests);
                      const canSelect = !isBooked && !isTooSmall;
                      ...
                      return (
                        <button
                          ...
                          className={`
                            ...
                            ${isSelected
                              ...
                              : isBooked
                              ? "bg-red-200 text-red-800 border-red-600 cursor-not-allowed"
                        ...
                        >
                          ...
                          {isBooked && <div className="text-red-600 font-bold text-xs mt-1">BOOKED</div>}
                          {isTooSmall && !isBooked && <div className="text-orange-600 text-xs">Too small</div>}
                          {isSelected && (
              ...
              <button
                type="submit"
                disabled={!selectedTable || isTableBooked(selectedTable?.number)}
                className={`w-full py-6 text-2xl font-bold rounded-2xl transition-all shadow-2xl mt-10 ${
                  selectedTable && !isTableBooked(selectedTable.number)
