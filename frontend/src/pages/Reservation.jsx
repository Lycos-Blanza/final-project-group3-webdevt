// src/pages/Reservation.jsx → FINAL 100% FRONTEND-ONLY (NO BACKEND)
...
const isOverlapping = (start1, start2) => {
  ...
  const [s1, e1] = [
    toMinutes(start1),
    toMinutes(start1) + RESERVATION_DURATION_MINUTES,
  ];
  const [s2, e2] = [
    toMinutes(start2),
    toMinutes(start2) + RESERVATION_DURATION_MINUTES,
  ];
  return s1 < e2 && s2 < e1;
};
...
export default function Reservation() {
  ...
  // ONLY CHANGE: use allReservations from context (stored in localStorage)
  const {
  ...
  const handleProceed = (e) => {
    e.preventDefault();
    if (!user)
      return notify("Please log in to continue", "info"), navigate("/login");
  ...
  const confirmAndPay = async () => {
    if (!selectedPayment)
      return notify("Please select payment method", "error");
    ...
    try {
      await createReservation({
        ...
        status: "Pending", // ← Now starts as "Pending" → Admin approves later
      });
  ...
  // REST OF YOUR CODE — 100% UNCHANGED (BEAUTIFUL UI)
  return (
          ...
          <div className="bg-white/97 backdrop-blur-xl rounded-3xl shadow-2xl p-10 lg:p-12 border border-white/40">
            ...
            <form onSubmit={handleProceed} className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="text-sm font-medium text-[#8B5A3C] mb-2 block">
                    Date
                  </label>
                  <input
                    type="date"
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-10">
            ...
            <div className="bg-[#FFF8F0] p-8 rounded-2xl mb-8 space-y-4 text-lg">
              ...
              <p>
                <strong>Table:</strong> T{selectedTable.number} (
