...
export default function AdminTables() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingId) {
      updateTable(editingId, { number: form.number, capacity: Number(form.capacity) });
      setEditingId(null);
    } else {
    ...
    <div className="pt-[56px] p-6 max-w-4xl mx-auto mt-[64px]">
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-4 bg-white rounded shadow">
        ...
        {editingId && (
          <button
            type="button"
            onClick={() => { setEditingId(null); setForm({ number: '', capacity: '' }); }}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {tables.map(table => (
          <div key={table.id} className="p-4 bg-white rounded shadow flex justify-between items-center">
            <div>
